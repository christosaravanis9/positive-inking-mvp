...
2. Open the SQL Editor and run `docs/supabase-schema.sql` once -- it creates
   ...
   `voice_input_used` value; no new columns are needed for it. **If that
   migration has already been run too and you're only missing the
   2026-09-14 idea-tracking addition**, run
   `docs/supabase-migration-2026-09-14-idea-tracking.sql` instead -- it
   widens the `event` check constraint for the new `idea_added` value and
   adds its four bucketing columns.
